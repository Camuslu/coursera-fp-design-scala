...
trait ObjectEncoder[-A] extends Encoder[A]:
  ...
  /**
    ...
    * Essentially it's a merging of 2 maps.
    */
...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromFunction { case x: Json.Num => if(x.value.isValidInt) Some(x.value.toInt) else None }

  /** A decoder for `String` values */
  given Decoder[String] =
    Decoder.fromFunction { case s: Json.Str => Some(s.value) }

  /** A decoder for `Boolean` values */
  given Decoder[Boolean] =
    Decoder.fromFunction { case b: Json.Bool => Some(b.value) }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case jsonArray: Json.Arr => {
        val items = jsonArray.items
        val maybeDecodedItems = items.map(decoder.decode)
        if (maybeDecodedItems.exists(_.isEmpty)) {
          None
        } else {
          Some(maybeDecodedItems.map(_.get))
        }
      }
    }
  ...
  /**
    ...
    * the [[A]] itself can also be a String/Int
    */
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction{ case obj: Json.Obj => obj.fields.get(name).flatMap(decoder.decode) }
...
trait ContactsCodecs:
  summon[Encoder[Person]]
  summon[Encoder[List[Person]]]
